...
const { teacherQueues } = useClassboardQueues(allTeachers, bookingsForSelectedDate);
...
const hasRealEvents = Object.values(newData).some((bookingData) =>
    bookingData.lessons.some((lesson) => lesson.id === optimisticEvent.lessonId && lesson.events?.length > 0),
);
```
