...
2.  **Missing Singleton Pattern**: In Next.js development (Hot Module Replacement), every file change or fast-refresh re-executes the module. Without a global singleton, this created a _new_ database client (with its own connection pool) on every reload, rapidly exhausting Supabase's connection limit.
