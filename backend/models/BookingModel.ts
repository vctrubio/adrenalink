...
export function createBookingModel(bookingData: any): BookingModel {
    ...
    const studentFirstNames = bookingStudents?.map((bs: any) => bs.student?.firstName || "").filter(Boolean).join(" ") || "";
    const studentLastNames = bookingStudents?.map((bs: any) => bs.student?.lastName || "").filter(Boolean).join(" ") || "";
    const studentPassports = bookingStudents?.map((bs: any) => bs.student?.passport || "").filter(Boolean).join(" ") || "";
    const studentPhones = bookingStudents?.map((bs: any) => bs.student?.phone || "").filter(Boolean).join(" ") || "";
