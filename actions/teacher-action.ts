...
import { teacher, lesson, teacherEquipment, teacherCommission, type LessonType, type EventType, type TeacherCommissionType } from "@/drizzle/schema";
...
> {
    try {
        ...
        if (schoolId) {
            const filteredLessons = lessonsWithEquipment.filter((item) => item.lesson.schoolId === schoolId);
...
export async function getTeacherCommissions(teacherId: string): Promise<
    ApiActionResponseModel<
        {
...
export async function getTeacherEquipment(teacherId: string): Promise<
    ApiActionResponseModel<
        {
            id: string;
            equipmentId: string;
...
export async function getTeacherEvents(teacherId: string, schoolId?: string): Promise<ApiActionResponseModel<any[]>> {
    try {
        ...
        const filteredLessons = schoolId ? lessonsResult.filter((l) => l.schoolId === schoolId) : lessonsResult;
