...
import { booking, bookingStudent, studentPackageStudent, schoolStudents, lesson, event } from "@/drizzle/schema";
import { createStudentModel, createBookingModel, type StudentModel, type BookingModel, type StudentPackageModel } from "@/backend/models";
...
export async function getStudentsBySchool(schoolId: string): Promise<ApiActionResponseModel<StudentModel[]>> {
    try {
        ...
        const students: StudentModel[] = result.map((ss) => createStudentModel(ss.student));
...
export async function getStudentBookings(studentId: string, schoolId?: string): Promise<ApiActionResponseModel<BookingModel[]>> {
    try {
        ...
        const studentBookings = bookingsResult.filter((b) => bookingIds.includes(b.id));

        const bookings: BookingModel[] = studentBookings.map((bookingData) => createBookingModel(bookingData));
...
export async function getStudentRequests(studentId: string, schoolId?: string): Promise<ApiActionResponseModel<StudentPackageModel[]>> {
    try {
        ...
        if (schoolId) {
            filteredPackages = filteredPackages.filter((pkg) => pkg.relations?.schoolPackage?.schoolId === schoolId);
        }

        const packages: StudentPackageModel[] = filteredPackages.map((packageData) => createStudentPackageModel(packageData));
