...
/**
 * Register Data Model
 * Consolidated data structure for the register page
 */
...
/**
 * Get all register data in a single optimized query
 * 
 * This action consolidates school info, packages, teachers (with commissions),
 * and students into a single database query using Drizzle's relation system.
 * 
 * Benefits:
 * - Single server round-trip
 * - Parallel data fetching at database level
 * - Type-safe relations via Drizzle
 * - Reduced network overhead
 * 
 * @returns RegisterData with all necessary information for the register form
 */
export async function getRegisterData(): Promise<ApiActionResponseModel<RegisterData>> {
    try {
        ...
        // Fetch teachers separately with commissions (not a direct school relation)
        const teachers = await db.query.teacher.findMany({
            ...
            orderBy: (fields, { asc }) => [asc(fields.username)],
        });

        // Fetch active referrals for the school
        const referrals = await db.query.referral.findMany({
            where: (fields, { eq, and }) => and(
                eq(fields.schoolId, schoolData.id),
        ...
        const registerData: RegisterData = {
            ...
            packages: schoolData.schoolPackages,
            teachers: teachers,
