                ...
                if (data.status === "OK" && data.result) {
                    ...
                    if (lat && lng) {
                        try {
                            ...
                            const tzResponse = await fetch(`/api/places/timezone?lat=${lat}&lng=${lng}`);
                            const tzData = await tzResponse.json();

                            if (tzData.timeZoneId) {
                                fetchedTimezone = tzData.timeZoneId;
                                console.log("✅ Timezone fetched:", fetchedTimezone);
                            } else {
                                console.warn("⚠️ No timeZoneId in response:", tzData);
                                fetchedTimezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
                                console.log("⚠️ Using browser timezone as fallback:", fetchedTimezone);
                            }
                        } catch (error) {
            ...
            {selectedPlace && (
                <div className="p-4 bg-accent/20 border border-accent rounded-md">
                    ...
                    <div className="text-sm space-y-1">
                        ...
                        {googlePlaceId && (
                            <div>
                                <strong>Place ID:</strong> {googlePlaceId}
                            </div>
                        )}
                    </div>
                </div>
