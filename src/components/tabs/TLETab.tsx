...
const EventItem = ({
	...
	event: ActiveBookingModel["events"][0];
	statusColor: string;
	statusLabel: string;
}) => {
	return (
...
const TeacherHeader = ({
	...
	return (
		<button
			onClick={onClick}
			className="w-full flex items-center justify-between gap-3 px-4 py-2 transition-colors cursor-pointer hover:opacity-80"
		>
			...
			{isExpanded && (
				<CommissionTable commission={commission} commissionColor={commissionColor} packageColor={packageColor} />
			)}
		</button>
	);
};

export const TLETab = ({ booking, isCollapsed = false, teacherId }: TLETabProps) => {
	...
	const filteredEvents = teacherId
		? booking.events.filter((event) => event.teacher?.id === teacherId)
		: booking.events;
							...
							<div className="space-y-2 pl-4 py-2 px-4 border-t" style={{ borderColor: teacherColor }}>
								{events.map((event) => (
									<EventItem
										key={event.id}
										event={event}
										statusColor={getEventStatusColor(event.status)}
