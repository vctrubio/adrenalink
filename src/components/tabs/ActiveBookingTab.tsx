...
const BookingProgressBar = ({ completedMinutes, events, booking }: { completedMinutes: number; events: any[]; booking: ActiveBookingModel }) => {
    ...
    return (
        <div className="flex items-center gap-3 flex-1">
            <div className="h-6 bg-muted rounded-full overflow-hidden flex-1 relative">
                <div className="h-full transition-all duration-300" style={{ width: `${progressPercent}%`, backgroundColor: progressColor }} />
                <div className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-muted-foreground font-medium">{getPrettyDuration(completedMinutes)}</div>
            </div>
