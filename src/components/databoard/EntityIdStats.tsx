...
interface EntityIdStatsProps {
    stats: StatItem[];
}

export const EntityIdStats = memo(function EntityIdStats({ stats }: EntityIdStatsProps) {
                ...
                <div key={index} className="flex-shrink-0 group cursor-default transition-all duration-300 hover:scale-105">
                    ...
                    >
                        ...
                        <div className="flex items-center gap-1" style={{ color: stat.color }}>
                            ...
                            <span className="text-lg font-bold">{typeof stat.value === "number" ? getCompactNumber(stat.value) : stat.value}</span>
                        </div>
                    </div>
                </div>
            ))}
