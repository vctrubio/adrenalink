...
import { getRelativeDateLabel, formatDateForInput, getTodayDateString, addDays } from "@/getters/date-getter";
...
export function ClassboardDatePicker({
    ...
    return (
        <div
            className={`flex flex-col items-center justify-center space-y-4 bg-background w-full ${disabled ? "opacity-50 pointer-events-none" : ""}`}
        >
            ...
            <div className="flex items-center gap-4">
                ...
                {isMounted && relativeLabel && (
                    <span className="text-sm text-muted-foreground">{isToday ? "Today" : relativeLabel}</span>
                )}
            </div>
        </div>
    );
