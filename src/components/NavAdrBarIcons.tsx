...
import SchoolIconButton from "@/src/components/ui/SchoolIconButton";
...
function SettingsDropdown() {
    ...
    return (
        <div className="relative">
            <ToggleSettingIcon isOpen={isOpen} onClick={() => setIsOpen(!isOpen)} />
                ...
                {isOpen && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="absolute top-14 right-0 p-2 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-2xl border border-white/20 dark:border-white/10 rounded-2xl flex flex-col gap-1 min-w-[200px] shadow-2xl z-[110]"
                        >
                            <button
                                onClick={toggleTheme}
                                className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-neutral-200/50 dark:hover:bg-white/10 transition-all group text-left"
                            >
...
export function NavIconsSkeleton() {
    return (
        <>
            {/* Desktop skeleton */}
            <motion.div
...
export function NavIconsRightSkeleton() {
    return (
        <>
            {/* Desktop */}
            <motion.div
...
// Right slot - Settings + School (mobile overlap)
interface RightIconsProps {
    credentials: SchoolCredentials | null;
}

export function RightIcons({ credentials }: RightIconsProps) {
    return (
        <>
            ...
            {/* Mobile: School in front, Settings overlapping */}
            <div className="md:hidden flex items-center relative">
                {credentials && (
                    <motion.div
                        initial={{ opacity: 0, scale: 0.8 }}
