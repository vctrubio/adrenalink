...
import { ArrowRight, Activity, TrendingUpDown, TrendingUp } from "lucide-react";
...
export function StatsPreview({ pillarId, showDescription = false }: { pillarId: string; showDescription?: boolean }) {
    const [isIndependent, setIsIndependent] = useState(false);
    const [isTeacherActive, setIsTeacherActive] = useState(true);

    const isPackage = pillarId === "package" || pillarId === "schoolPackage";
        ...
        <div className="flex flex-col gap-12">
            ...
            {showDescription && (
                <div className="max-w-2xl">
                    <p className="text-lg text-muted-foreground leading-relaxed tracking-wide italic">
                        {pillarId === "student" && "Comprehensive tracking for every student's journey, from initial registration to progress  and booking history."}
                        {pillarId === "teacher" && "Manage your team's availability. Calculate commissions automatically based on lesson hours and revenue generated."}
                        {pillarId === "booking" && "The core of your operations. Real-time lesson synchronization that coordinates students, teachers, and equipment while tracking payments and revenue."}
                        {pillarId === "equipment" && "Maximize the lifecycle of your inventory. Track repairs, assign gear to specific teachers, and know when to say goodbye."}
                        {isPackage && "Customize your pricing strategies with internal or public package options, allowing you to define durations, capacities, and rates for any activity."}
                    </p>
                </div>
            )}
