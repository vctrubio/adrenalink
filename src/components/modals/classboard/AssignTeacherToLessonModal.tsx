...
export function AssignTeacherToLessonModal({ isOpen, onClose, bookingData, onAssigned }: AssignTeacherToLessonModalProps) {
    ...
    const modalTitle =
        studentCount > 1 ? `${bookingData.booking.leaderStudentName} +${studentCount - 1}` : bookingData.booking.leaderStudentName;

    // Get existing teacher+commission combos in this booking
    const existingCombos = useMemo(() => {
        return new Set(
            bookingData.lessons.filter((l) => l.teacher?.id && l.commission?.id).map((l) => `${l.teacher!.id}:${l.commission!.id}`),
        );
    }, [bookingData.lessons]);

    // Get selected teacher's data
    const selectedTeacher = selectedTeacherId ? teachers.find((t) => t.schema.id === selectedTeacherId) : null;
    ...
    const comboAlreadyExists =
        selectedTeacherId && selectedCommissionId ? existingCombos.has(`${selectedTeacherId}:${selectedCommissionId}`) : false;

    const canAssign = selectedTeacherId && selectedCommissionId && !comboAlreadyExists && !isAssigning;
    ...
    const handleAssign = useCallback(async () => {
        ...
        setIsAssigning(true);
        try {
            const result = await createLessonWithCommission(bookingData.booking.id, selectedTeacherId, selectedCommissionId);
                                ...
                                {selectedTeacherCommissions.length > 0 && (
                                    <div className="space-y-2">
                                        {selectedTeacherCommissions.map((commission) => {
                                            const comboExists = existingCombos.has(`${selectedTeacherId}:${commission.id}`);
