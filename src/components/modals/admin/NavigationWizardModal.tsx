...
export function NavigationWizardModal({ isOpen, onClose }: NavigationWizardModalProps) {
    ...
    const allEntities = useMemo(
        () => TARGET_ENTITIES.map((id) => ENTITY_DATA.find((e) => e.id === id)).filter((e): e is (typeof ENTITY_DATA)[number] => !!e),
        [],
    );
                                    ...
                                    {!expandedEntityId ? (
                                        ...
                                        >
                                            <PopUpSearch
                                                ...
                                                onChange={mainNav.setSearchQuery}
                                                className="mb-4"
                                                    ...
                                                    <PopUpRows
                                                        ...
                                                        renderItem={(item, isSelected) => (
                                                            <div className="flex items-center justify-between px-4 py-3 gap-4">
                                                                <div className="flex flex-col">
                                                                    <span
                                                                        className={`font-bold transition-colors ${isSelected ? "text-primary dark:text-secondary" : "text-foreground"}`}
                                                                    >
                                                                        {item.title}
                                                                    </span>
                                                                ...
                                                                <div
                                                                    className={`px-2 py-0.5 rounded-full text-[10px] uppercase font-black tracking-widest ${item.isActive ? "bg-green-500/10 text-green-600 dark:text-green-400" : "bg-muted/30 text-muted-foreground"}`}
                                                                >
                                    ...
                                    <div className="flex items-center justify-between px-3 py-1.5 rounded-lg bg-muted/20 border border-border/40">
                                        <AnimatePresence mode="wait">
                                            {!expandedEntityId ? (
                                                ...
                                                >
                                                    <span className="text-[9px] font-black text-muted-foreground/60 uppercase tracking-widest truncate mr-2">
                                                        Browse
                                                    </span>
                                                    <span className="bg-background px-1.5 py-0.5 rounded shadow-sm text-foreground font-black text-[10px]">
                                                        TAB
