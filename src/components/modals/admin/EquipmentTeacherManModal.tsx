...
export function EquipmentTeacherManModal({ isOpen, onClose, equipment }: EquipmentTeacherManModalProps) {
    ...
    const linkedTeacherIds = new Set(equipment.relations?.teachers?.map((t: any) => t.id).filter(Boolean) || []);
        ...
        >
            <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <PopUpSearch value={searchQuery} onChange={setSearchQuery} className="flex-1 min-w-[200px]" />
                    ...
                    {filteredTeachers.length > 0 && (
                        <div
                            className="flex-shrink-0 px-3 py-2.5 rounded-lg text-sm font-medium popup-text-secondary"
                            style={{ backgroundColor: `${teacherEntity?.color}15` }}
                        >
