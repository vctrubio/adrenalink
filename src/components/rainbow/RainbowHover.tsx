...
const RainbowHoverBody = ({ hoveredShade }: { hoveredShade: RainbowShade }) => {
    ...
    return <div className="px-6 py-4 border-b border-white/10">{Description && <Description />}</div>;
};
    ...
    return (
        <div className="px-6 py-4 flex flex-wrap gap-2">
            {entitiesForColor.map((entity) => {
                const rainbowEntity = RAINBOW_ENTITIES.find((e) => e.id === entity.id);
                return rainbowEntity ? (
                    <RainbowTag key={entity.id} entity={entity} shade={rainbowEntity.shadeId as RainbowShade} />
                ) : null;
            })}
        </div>
    );
