            ...
            <div className="mb-6 p-4 border border-border rounded-lg bg-card">
                ...
                <div className="mb-4">
                    <div className="flex gap-4">
                        <button
                            ...
                            className={`px-4 py-2 rounded border transition-colors ${logic.inputMode === "file" ? "bg-primary text-primary-foreground border-primary" : "bg-background text-foreground border-border hover:bg-accent hover:text-accent-foreground"}`}
                        >
                            Upload File
                        </button>
                        <button
                            onClick={() => logic.handleInputModeChange("text")}
                            className={`px-4 py-2 rounded border transition-colors ${logic.inputMode === "text" ? "bg-primary text-primary-foreground border-primary" : "bg-background text-foreground border-border hover:bg-accent hover:text-accent-foreground"}`}
                        >
                            Paste Text
                        </button>
                    </div>
                ...
                {logic.inputMode === "file" ? (
                    ...
                    <div>
                        ...
                        />
                        {logic.textInput && <p className="mt-2 text-sm text-muted-foreground">Characters: {logic.textInput.length}</p>}
                    </div>
                )}
            </div>
                    ...
                    <div className="mb-6 p-4 border border-border rounded-lg bg-card">
                        ...
                        <div className="flex gap-2 items-center flex-wrap">
                            {logic.SEPARATORS.map((sep) => (
                                <button
                                    ...
                                    className={`px-3 py-2 rounded border text-sm transition-colors ${
                                        logic.separator === sep.value ? "bg-primary text-primary-foreground border-primary" : "bg-background text-foreground border-border hover:bg-accent hover:text-accent-foreground"
                                    }`}
                                >
                                    {sep.label}
                                </button>
                            ...
                            <button
                                ...
                                className={`px-3 py-2 rounded border text-sm transition-colors ${
                                    logic.separator === "custom" ? "bg-primary text-primary-foreground border-primary" : "bg-background text-foreground border-border hover:bg-accent hover:text-accent-foreground"
                                }`}
                            >
                                Custom
                            </button>
                    ...
                    <div className="mb-6 p-4 border border-border rounded-lg bg-card">
                        ...
                        <div className="mb-4">
                            <label className="flex items-center space-x-2">
                                <input type="checkbox" checked={logic.hasHeaders} onChange={(e) => logic.handleHasHeadersChange(e.target.checked)} className="rounded" />
                        ...
                        <div className="mb-4">
                            <label className="flex items-center space-x-2">
                                <input type="checkbox" checked={logic.useCustomHeaders} onChange={(e) => logic.setUseCustomHeaders(e.target.checked)} className="rounded" />
                        ...
                        {logic.parsedData && (
                            <div>
                                <h3 className="font-medium mb-3">Detected Columns ({logic.parsedData.headers.length}):</h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                    {logic.parsedData.headers.map((header, index) => (
                                        <div key={index} className="flex flex-col">
                                            <label className="text-sm text-muted-foreground mb-1">Column {index + 1}</label>
                                            {logic.useCustomHeaders ? (
                                                ...
                                                <div className="px-3 py-2 bg-muted border border-border rounded text-muted-foreground">{header || `Column ${index + 1}`}</div>
                                            )}
                                        </div>
                                    ))}
                                </div>
                    ...
                    {logic.parsedData && (
                        <div className="mb-6 p-4 border border-border rounded-lg bg-card">
                            <div className="flex justify-between items-center mb-4">
                                <div>
                                    ...
                                    <div className="flex items-center gap-4 mt-2">
                                        ...
                                        <button onClick={logic.selectAllRows} className="text-xs px-2 py-1 bg-accent text-accent-foreground rounded hover:bg-accent/80 transition-colors">
                                            Select All
                                        </button>
                                        <button onClick={logic.deselectAllRows} className="text-xs px-2 py-1 bg-accent text-accent-foreground rounded hover:bg-accent/80 transition-colors">
                                            Deselect All
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div className="mb-4 text-sm text-muted-foreground">
                                Found {logic.parsedData.rows.length} rows and {logic.parsedData.headers.length} columns
                            </div>
                            ...
                            <div className="overflow-x-auto rounded-lg border border-border">
                                <table className="min-w-full">
                                    <thead className="bg-muted">
                                        <tr>
                                            <th className="px-4 py-3 border-b border-muted/30 text-left text-sm font-semibold text-foreground">Row #</th>
                                            {(logic.useCustomHeaders ? logic.customHeaders : logic.parsedData.headers).map((header, index) => (
                                                <th key={index} className="px-4 py-3 border-b border-muted/30 text-left text-sm font-semibold text-foreground">
                                    ...
                                    <tbody>
                                        {logic.parsedData.rows.slice(0, 10).map((row, rowIndex) => (
                                            <tr key={rowIndex} className={`hover:bg-accent/30 transition-colors border-b border-muted/30 ${logic.selectedRows.has(rowIndex) ? "bg-primary/10 border-primary/20" : ""}`}>
                                                <td className="px-4 py-3 text-sm font-medium cursor-pointer hover:bg-accent/50 transition-colors" onClick={() => logic.toggleRowSelection(rowIndex)} title="Click to toggle row selection">
                                                    <div
                                                        className={`flex items-center justify-center w-6 h-6 rounded border transition-colors ${
                                                            logic.selectedRows.has(rowIndex) ? "bg-primary text-primary-foreground border-primary" : "text-muted-foreground border-border hover:border-primary/50"
                                                        }`}
                                                    >
                                                        {rowIndex + 1}
                                                    </div>
                                                    ...
                                                    return (
                                                        <td key={cellIndex} className="px-2 py-3">
                                                            <input
                                                                type="text"
                                                                value={cell}
                                                                onChange={(e) => logic.handleCellChange(rowIndex, cellIndex, e.target.value)}
                                                                className={`w-full px-2 py-1 text-sm bg-transparent border rounded focus:outline-none focus:ring-2 focus:ring-ring transition-colors ${
                                                                    isEdited ? "border-orange-500 bg-orange-50 dark:bg-orange-950/20" : "border-transparent hover:border-border focus:border-input"
                                                                }`}
                                                            />
                                                        </td>
                                                    );
                            ...
                            {logic.parsedData.rows.length > 10 && <p className="mt-3 text-sm text-muted-foreground">Showing first 10 rows of {logic.parsedData.rows.length} total rows (editable)</p>}
                        </div>
                    )}

                    <div className="flex justify-center">
                        <button onClick={logic.handleSubmit} className="px-8 py-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 focus:ring-2 focus:ring-ring focus:ring-offset-2 font-semibold transition-colors">
